...
class ErrorsCog(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            try:
                await ctx.send(
                    f"Bot doesn't have the required `{', '.join(error.missing_permissions)}` permission(s).",
                    delete_after=5,
                )
                await ctx.message.delete()
            except Exception:
            ...
            await ctx.send(
                f'Member "{error.argument}" is not in this server or doesn\'t exist. Try again!'
            )
            ...
            await ctx.send(
                f"Chill! This command is on cooldown for `{round(error.retry_after)}` seconds."
            )
